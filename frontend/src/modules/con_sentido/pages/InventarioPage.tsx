...
export function InventarioConSentidoPage() {
  ...
  function guardarProducto(producto: any) {
    const nuevosProductos = [producto, ...productos];
    setProductos(nuevosProductos);
    localStorage.setItem("consentido_inventario_productos", JSON.stringify(nuevosProductos));
  }
            ...
            <tbody>
              {productos.map((producto) => (
                <tr key={producto.id} className="border-t border-brand-vanilla-dark dark:border-brand-green-700">
                  ...
                  <td className="px-4 py-2 text-xs text-brand-ink/60 dark:text-brand-vanilla/60">{producto.sku}</td>
                  <td className="px-4 py-2">
                    <span className={`rounded px-2 py-1 text-xs font-medium ${
                      producto.stock <= 0 ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                      producto.stock <= 5 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' :
